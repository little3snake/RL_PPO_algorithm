...
class PPO:
    ...
    def learn(self):
        ...
        for i in range(1, self.num_iterations + 1):
            ...
            #print(f"Rank {self.rank}: Starting iteration {i}", flush=True)
            ...
            #if dist.get_rank() == 1:
            #    print(f"Rank 1: Finished rollout and ready for synchronization.", flush=True)
            #dist.barrier()  # Оба процесса должны дойти до этой точки

            #if self.rank == 0:
            #print(f" {self.rank}               Check 1 Before update model.")
            try:
                #print (f" {self.rank} start to ")
            ...
            #    print(
            #        f"Rank {local_rank}: GPU memory allocated after iteration {i}: {torch.cuda.memory_allocated() / 1e6:.2f} MB")
            #    print(
            #        f"Rank {local_rank}: GPU memory reserved after iteration {i}: {torch.cuda.memory_reserved() / 1e6:.2f} MB")
        ...
        try:
            ...
            for _ in range(self.update_epochs):
                ...
                if self.world_size > 1:
                    ...
                    #print(
                    #    f"Rank {dist.get_rank()}: Ready to reduce. all_actor_grads shape: {all_actor_grads.shape}, device: {all_actor_grads.device}",
                    #    flush=True)
