...
class PPO:
    def __init__(self, policy_class, env, device, writer, **kwargs):
        ...
        self.timesteps_per_batch = kwargs.get('timesteps_per_batch', 4800)
        ...
        self.num_minibatches = kwargs.get('num_minibatches', 6)
        self.minibatch_size = 0 # batch_size / num_minibatches  - init in rollout - end
        self.max_grad_norm = kwargs.get('max_grad_norm', 0.5)
        self.entropy_coef = kwargs.get('entropy_coef', 0.01)
        self.clip_epsilon = kwargs.get('clip_epsilon', 0.2)
        self.num_iterations = kwargs.get('num_iterations', 400)
        self.num_episodes_in_rollout = self.timesteps_per_batch // self.max_timesteps_per_episode
        ...
        self.memory = Memory(device=self.device, obs_size=self.env.observation_space.shape[0], action_size=self.env.action_space.shape[0],
                             **kwargs)
        self.timesteps_in_current_batch = 0 # init in rollout (in the end)
    ...
    def learn(self, total_timesteps):
        ...
        while timesteps_current < total_timesteps:
            ...
            batch_inds = np.arange(self.timesteps_in_current_batch)
            for _ in range(self.n_updates_per_iteration):  # ALG STEP 6 & 7
                np.random.shuffle(batch_inds)
                for i in range (0, self.timesteps_in_current_batch, self.minibatch_size):
                #V, current_log_probs, entropy = self.evaluate(batch_observations, batch_actions)
                    #print ("current_batch_size", self.timesteps_in_current_batch)
                    #print("current_minibatch_size", self.minibatch_size)

                    start = i
                    finish = i + self.minibatch_size
                    minibatch_inds = batch_inds[start:finish]
                    if len(minibatch_inds) < 2:
                        break
                    #print ("minibatch_inds", minibatch_inds)
    ...
    def rollout(self, timestep_current):
        ...
        current_timestep = -1 # for 0 in first step (because of +1)

        #for current_timestep in range(self.timesteps_per_batch):
        #for current_episode in range(self.num_episodes_in_rollout):
        ...
        self.timesteps_in_current_batch = self.memory.get_current_batch_size()
        self.minibatch_size = self.timesteps_in_current_batch // self.num_minibatches  # batch_size / num_minibatches
